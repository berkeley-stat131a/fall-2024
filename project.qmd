...
## Final project options (DRAFT+SUBJECT TO CHANGE)
...
- Patients were randomly given either `D-penicillamine` or a placebo (variable `drug`), and both the patient and the health care providers were unaware as to which the patient received ("double-blinded"). 

There are 418 observations of 20 variables, both numeric and categorical. 

- The description of the variables is found in `cholangitis_dictionary.csv`.
...
- However, in the final submission, you will submit 5-7 professionally-formatted visualizations that each showcase a non-trivial finding from the data.
...
- There should be evidence that you considered interactions and polynomial terms when fitting your final models, though they do not necessarily have to be contained in the models.

- You should use cross-validation to select the best performing model specifications. You are free to choose any error metric to validate on. For example, you might choose RMSE, or another metric that you think is more appropriate given the dataset. 
